...
#########################
ai_name = 'K.A.W.A.I'.lower()
EXIT_COMMANDS = ['bye','exit','quit','shut down', 'shutdown', 'switchoff']
...
voice_id = 1 #0 for female, 1 for male
...
try:
	...
	from web_scrapping import play_on_youtube
...
def main(text):
		...
		if isContain(text, ['file', 'explorer']):
			def explorer():
				os.system('explorer')
		...
		if isContain(text, ['download', 'folder']):
			def download():
				os.startfile("C:\\Users\\Vedan\\Downloads")
		...
		if isContain(text, ['what is','temperature']):
			import requests
			from bs4 import BeautifulSoup
			def temperature():
				search = "temperature in ghansoli"
				url = f"https://www.google.com/search?q={search}"
				r = requests.get(url)
				data = BeautifulSoup(r.text, "html.parser")
				temp = data.find("div",class_="BNeawe").text
				speak(f"current {search} is {temp}", True,True)
			temperature()
			return
...
if __name__ == '__main__':
	...
	n = StringVar()
	assVoiceOption = ttk.Combobox(settingsFrame, values=('Female', 'Male'), font=('Arial', 13), width=10, textvariable=n)
	assVoiceOption.current(voice_id)
	assVoiceOption.place(x=160, y=20)
